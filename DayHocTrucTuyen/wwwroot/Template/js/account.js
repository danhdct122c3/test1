...
function isPasswordStrong(password) {
    // Mật khẩu phải dài ít nhất 8 ký tự, có chữ cái thường, chữ cái hoa, số và ký tự đặc biệt
...
$('#formCreate').on('submit', function () {
    ...
    const password = $('#passCreate').val();
    if (!isPasswordStrong(password)) {
        $('#passCreate').css('border-color', 'red');
        $('#erroCreate').text("Mật khẩu phải dài ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt.");
        $('#erroCreate').show('slow');
        $('#erroCreate').delay(2000).hide('slow');
        return;
    }
